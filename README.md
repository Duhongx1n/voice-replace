...
当你的新台词是一整段文字（而非按原视频逐句编写）时，工具会**自动检测并调用大模型**进行智能分句（已内置 HAI 平台 API Key，无需额外配置）：
...
> 工具会自动检测新台词是否需要适配。如果你的台词已经按原视频逐句编写，则不会调用大模型。
...
打开 `output/transcript_for_edit.md`，你会看到类似这样的内容：

```markdown
# 视频台词编辑
# 每行对应原视频的一个语句片段，请逐行替换为新台词。
# 行数应与原始片段数一致（5 行），以确保时间对齐。
# 以 # 开头的行会被忽略。
...
# --- 第 3 句 [7.50s - 12.00s] 原文: 首先让我们看看它的发展历程
首先让我们看看它的发展历程
```
...
- **保持行数一致**可获得最佳时间对齐效果
- 行数多于原始片段时，多余台词会紧接在最后一句之后
- 行数少于原始片段时，多余的时间槽保持静音
...
| OpenAI API（可选） | 智能分句 | 将自由格式台词按原视频节奏拆分 |
| HAI 大模型平台（内置） | 智能分句 | 已内置 DeepSeek-V3.1，开箱即用 |
...
**推荐方案：直接使用智能分句功能**。工具已内置 HAI 大模型平台的 API Key，无需额外配置：
...
如需使用其他大模型，可通过参数或环境变量覆盖：

```bash
# 使用外部 OpenAI
export OPENAI_API_KEY=sk-xxx
voice-replace --input video.mp4 --output_dir output \
    --new_text my_script.txt --llm_model gpt-4o-mini
```
