...
只需准备一段新台词文本，工具会**自动调用大模型**按原视频节奏智能拆分并生成新视频（已内置 HAI 平台 API Key，开箱即用，无需额外配置）：

```bash
# 最简用法 — 直接给一整段新台词，大模型自动适配！
...
> 你不需要关心原视频有几句话、每句话的时间点在哪里。大模型会自动分析原视频结构，将你的新台词智能拆分对齐。

### 手动模式（两步完成）

如果你希望精确控制每一句台词，也可以手动编辑：
